...
export function ProTipp({ proTipp }: ProTippProps) {
  ...
  return (
    ...
    >
      <div className="bg-gradient-to-br from-[#FEF9EC] to-[#FEF3D0] px-5 py-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <p className="font-black text-[#B07D0A] text-sm uppercase tracking-widest">Pro-Tipp</p>
        </div>
        <p className="text-[#1A1A1A] text-sm leading-relaxed font-medium">{proTipp.text}</p>
        {proTipp.extraIngredients && proTipp.extraIngredients.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-4">
            {proTipp.extraIngredients.map((extra) => (
              <div
                key={extra.name}
                className="flex items-center gap-2 bg-white border border-[#F2A20C]/40 rounded-full px-3 py-1.5 shadow-sm"
                title={extra.reason}
              >
                <IconCartPlus className="w-3 h-3 text-[#B07D0A]" />
                <span className="text-xs font-bold text-[#1A1A1A]">{extra.name}</span>
                <span className="text-xs text-[#6B7870]">einkaufen</span>
              </div>
            ))}
          </div>
        )}
